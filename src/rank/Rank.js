...
const { formatVariable, applyText, randomColor } = require("../../utils/functions");
...
module.exports = class RankCard {
  ...
  async toAttachment() {
    ...
    ctx.strokeText(this.username, 50 + 240 + 45 + 5, 80);
    ...
    if (this.addonRank) {
      ...
      ctx.strokeText("#" + this.rank, canvas.width - 50 - 5, 80);
    }
    if (this.addonRankName) {
      ...
      ctx.strokeText(this.rankName, 50 + 240 + 45 + 5, 80 + 45 + 15);
    }
    ...
    //set canvas 50% blurry to make it look like it's being drawn
    ctx.imageSmoothingEnabled = false;

    ctx.font = applyText(canvas, textXPEdited, 28, 690, "Bold");
    ctx.fillText(
      textXPEdited,
      50 + 240 + 45 + 5 + 690 / 2,
      80 + 45 + 15 + 30 + 90
  ...
  async toAttachmentCustom(bgColor, bgOpacity) {
    let canvas = Canvas.createCanvas(1080, 400),
      ctx = canvas.getContext("2d");
    ...
    ctx.closePath();
    ctx.clip();
    ctx.fillStyle = bgColor; // iki nyok
    ctx.fillRect(0, 0, 1080, 400);
    let background = await Canvas.loadImage(this.backgroundImage);
